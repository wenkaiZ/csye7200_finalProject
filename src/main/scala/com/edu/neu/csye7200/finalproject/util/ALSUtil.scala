...
object ALSUtil {
  ...
  /**
    ...
    */
  def makeRecommendation(apps: Map[Int, String], userRating: RDD[Rating]) = {
    val appId = userRating.map(_.product).collect.toSeq
    val candidates = DataUtil.spark.sparkContext.parallelize(apps.keys.filter(!appId.contains(_)).toSeq)
    bestModel.get
  ...
  /**
    * Combine all the function above and print out the recommendation apps
    * @param trainSet       RDD of [[]Rating]] objects of training set
    * @param validationSet  RDD of [[]Rating]] objects of validation set
    * @param testSet        RDD of [[]Rating]] objects of test set
    * @param apps         Maps of app which appsId as key and title as value
  ...
  def trainAndRecommendation(trainSet: RDD[Rating], validationSet: RDD[Rating], testSet: RDD[Rating]
                             , apps: Map[Int, String], userRating: RDD[Rating]) ={
    trainAndOptimizeModel(trainSet, validationSet)
    val RMSE = evaluateMode(trainSet, validationSet, testSet)
    val recommendations = makeRecommendation(apps, userRating)
    var i = 1
    println( "Apps recommended for you:")
    recommendations.foreach{ line=>
      println("%2d".format(i)+" :"+apps(line.product))
      i += 1
    }
    RMSE
  }
